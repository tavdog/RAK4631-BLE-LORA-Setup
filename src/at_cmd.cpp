...
static int at_exec_p2p_receive(char *str)
{
	...
	if (param != NULL)
	{
		...
		APP_LOG("AT", "Received RX time %ld", rx_time);
		...
		else if (rx_time == 65535)
		{
			...
			// Put Radio into continuous RX mode
			Radio.Rx(0);
			APP_LOG("AT", "Set RX_MODE_RX_WAIT");
		}
		else if (rx_time < 65534)
		{
			...
			// Put Radio into continuous RX mode
			Radio.Rx(rx_time);
			APP_LOG("AT", "Set RX_MODE_RX_TIMED");
		}
		else
		{
			return AT_ERRNO_PARA_VAL;
...
static int at_exec_join(char *str)
{
	...
	if (param != NULL)
	{
		...
		{
			APP_LOG("AT", "autoJoin = %d", autoJoin);
			return AT_ERRNO_PARA_VAL;
		}
		has_autojoin = true;
		param = strtok(NULL, ":");
...
static void at_cmd_handle(void)
{
	...
	// 			{
	// 				APP_LOG("AT", "Not a user AT command");
	// 				ret = AT_ERRNO_NOSUPP;
	// 			}
	// 		}
