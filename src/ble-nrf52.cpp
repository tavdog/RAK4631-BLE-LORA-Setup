...
void settings_rx_callback(uint16_t conn_hdl, BLECharacteristic *chr, uint8_t *data, uint16_t len)
{
	APP_LOG("SETT", "Settings received");
	...
	if (chr->uuid == g_lora_data.uuid)
	{
		...
		{
			APP_LOG("SETT", "Received settings have wrong size %d", len);
			return;
		}
		...
		{
			APP_LOG("SETT", "Received settings data do not have required markers");
			return;
		}
		...
		APP_LOG("SETT", "Waking up loop task");
	}
}
